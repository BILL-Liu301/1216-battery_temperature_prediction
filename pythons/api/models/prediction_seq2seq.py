...
class Prediction_Seq2seq_Model(nn.Module):
    def __init__(self, paras: dict):
        ...
        # 对历史时序数据进行编码，his
        self.his_linear_layer_encoder_h = nn.Linear(in_features=1, out_features=self.size_middle, bias=self.bias)
        self.his_linear_layer_encoder_c = nn.Linear(in_features=1, out_features=self.size_middle, bias=self.bias)
        ...
        self.his_lstm = nn.LSTM(input_size=4, hidden_size=self.size_middle, num_layers=self.lstm_num_layers, bidirectional=self.lstm_bidirectional, bias=self.bias, batch_first=True)
        ...
        self.pre_lstm_m_var = nn.LSTM(input_size=3, hidden_size=self.size_middle, num_layers=self.lstm_num_layers, bidirectional=self.lstm_bidirectional, bias=self.bias, batch_first=True)
    ...
    def forward(self, inp_loc_i_soc_his, inp_temperature_his, inp_loc_i_soc, h_his=None, c_his=None):
        ...
        temperature_start_his = inp_temperature_his[:, 0:1]
        if (h_his is None) and (c_his is None):
            # 对历史数据进行编码
            # h0 = self.his_linear_layer_encoder_h(temperature_start_his.repeat(self.D * self.lstm_num_layers, 1))
            # c0 = self.his_linear_layer_encoder_c(temperature_start_his.repeat(self.D * self.lstm_num_layers, 1))
        ...
        oup_m_ = self.pre_linear_layer_decoder_m_(self.pre_norm_m_(lstmed_m_var)) + temperature_start_his
        oup_var = self.pre_linear_layer_decoder_var(self.pre_norm_var(lstmed_m_var))
