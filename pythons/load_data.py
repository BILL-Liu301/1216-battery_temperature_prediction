...
if __name__ == '__main__':
    ...
    # 基础存储单元
    # 时间[s]，位置，电流，SOC，测点1，测点2，测点3，测点4，测点5，测点6，测点7
    #         上
    #       1    2
    # 内  3    4   5  外
    #       6    7
    ...
    data_np_origin = np.zeros([duration.shape[0], 4 + num_measure_point])
    ...
    for cell_name, cell_location in temperature_location.items():
        ...
        data_np_origin[:, 2:4] = np.asarray(data_xlsx[['电流', 'SOC']])[0:duration.shape[0]]
        data_np_origin[:, 4:] = np.asarray(data_xlsx[temperature_measure_points[cell_name]])[0:duration.shape[0]] + K
